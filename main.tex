...
\begin{proof}
  ...
  For this we must show that if $\mathcal{C}$ is a lcc-marked category, then marked maps of the form $i(\mathrm{Pb}) \rightarrow \mathcal{C}$ are stable under isomorphisms and $\mathcal{C}$ is $\mathrm{pb}_i$-local for $i = 0, 1, 2$ if and only if the underlying category $U(\mathcal{C})$ has all pullbacks and maps $i(\mathrm{Pb}) \rightarrow U(\mathcal{C})$ are marked precisely when their images are pullback squares.
  Thus let us explicitly the condition on $\mathrm{pb}_i$-locality.
  \begin{itemize}
    \item
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item
      ...
      An object of $\mathrm{Lcc}$ is fibrant if and only if it has the right lifting property against the morphisms $j_K$ from \ref{itm:marked-fibrant-object-generators}, the morphisms of definition \ref{def:lcc-axioms}, and the canonical maps $\mathrm{tm}_2 \amalg \mathrm{tm}_2 \rightarrow \operatorname{cod} \mathrm{tm}_2$, $\mathrm{pb}_2 \amalg \mathrm{pb}_2 \rightarrow \operatorname{cod} \mathrm{pb}_2$ and $\mathrm{pi}_2 \amalg \mathrm{pi}_2 \rightarrow \operatorname{cod} \mathrm{pi}_2$.
  \end{enumerate}
\end{proposition}
\begin{proof}
  ...
  By proposition \ref{prop:lcc-model-cat}, the fibrant objects of $\mathrm{Lcc}$ are lcc categories with precisely the universal diagrams for terminal objects, pullback squares and dependent products marked, thus the maps induced by lifts against $\mathrm{tm}_2, \mathrm{pb}_2$ and $\mathrm{pi}_2$ are unique.
  ...
  Conversely, if a fibrant lcc-marked category has the right lifting property against $S$ and the lifts against $\mathrm{tm}_2, \mathrm{pb}_2$ and $\mathrm{pi}_2$ are unique, then it is an lcc category with appropriate markings and hence fibrant in $\mathrm{Lcc}$.
\end{proof}
...
\begin{proof}
  ...
  \begin{mathpar}
    ...
    \and
    \begin{tikzcd}
      f(\tau) \arrow[r, "{\langle w, \mathrm{id} \rangle}"] \arrow[d] & f(\sigma) \times f(\tau) \arrow[d, "f(\sigma^*(\tau))"] \\
      1 \arrow[r, "w"] & f(\sigma)
    \end{tikzcd}
  \end{mathpar}
