...
What's worse, our interpretation of contexts as lcc categories does not admit well-behaved context extensions.
Recall that for a context $\Gamma$ and type $\Gamma \vdash \sigma$ in a cwf, a context extension consists of a a context morphism $p : \Gamma.\sigma \rightarrow \Gamma$ and a term $\Gamma.\sigma \vdash v : \sigma[p]$, such that for every other morphisms $f : \Delta \rightarrow \Gamma$ and term $\Delta \sigma \vdash s : \sigma[f]$ there is a unique morphism $\langle f, s \rangle : \Delta \rightarrow \Gamma.\sigma$ over $\Gamma$ such that $v[\langle f, s \rangle] = s$.
...
Thus the diagonal $d : \sigma \rightarrow \sigma \times \sigma$ corresponds to a term $\Gamma_{/ \sigma} \vdash \sigma^*(\sigma)$.
The problem is that, while this data is indeed universal in a suitable sense, it is only so in the bicategorical sense (see Lemma \ref{lem:slice-is-weak-ext}).
Given any lcc functor $f : \Gamma \rightarrow \Delta$ and term $\Delta \vdash w : f(\sigma)$, we obtain an lcc functor
\begin{equation}
  \begin{tikzcd}
    \langle f, s \rangle : \Gamma_{/ \sigma} \arrow[r, "{f_{/ \sigma}}"] & \Delta_{/ f(\sigma)} \arrow[r, "s^*"] & \Delta,
  \end{tikzcd}
\end{equation}
but this functor commutes with $\sigma^*$ and $f$ up to a natural isomorphism, and the equation $\langle f, s \rangle(d) = s$ holds only up to this natural isomorphism.
