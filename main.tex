...
\begin{proof}
  ...
  For this we need to prove that if $f : S \rightarrowtail T$ is a cofibration of simplicial sets and $g : X \rightarrowtail Y$ is a cofibration of marked objects, then the canonical map $f \mathbin{\square} g$ in
  \begin{equation}
    \label{eq:tensor-pushout-product}
    \begin{tikzcd}
      S \otimes X \arrow[r] \arrow[d] \arrow[dr, phantom, near end, "\ulcorner"] & T \otimes X \arrow[d] \arrow[ddr, bend left=10] \\
      S \otimes Y \arrow[r] \arrow[drr, bend right=10] & \cdot \arrow[dr, dashed, "f \mathbin{\square} g" description, label] \\
      & & T \otimes Y
    \end{tikzcd}
  \end{equation}
  is a cofibration, and that it is a weak equivalence if at least one of $f$ and $g$ is furthermore a weak equivalence.
