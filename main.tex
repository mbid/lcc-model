...
\end{equation}
To interpret it, we would like to apply the dependent product functor $G(\Gamma)_{/ \sigma} \rightarrow G(\Gamma)$ of underlying categories to $\tau$.
But $\tau$ is an object of $\Gamma.\sigma$, so we will first need to construct a comparison map $D : G(\Gamma.\sigma) \rightarrow G(\Gamma)_{/ \sigma}$.
It is tempting to define $D$ as a strict lcc functor using the universal property of $\Gamma.\sigma$.
For this we first endow $G(\Gamma)_{/\sigma}$ with structure of a \emph{strict} lcc category:
\todo{
  For this we should know precisely what the generating trivial cofibrations of $\mathrm{Lcc}$ are.
}
The canonical terminal object of $G(\Gamma)_{/ \sigma}$ is the identity $\sigma \rightarrow \sigma$ on $\sigma$, the canonical pullbacks in $G(\Gamma)_{/ \sigma}$ are computed as canonical pullbacks in $G(\Gamma)$, and the canonical functors $\Pi : (G(\Gamma)_{/ \sigma})_{/ \tau_0} \rightarrow (G(\Gamma)_{/ \sigma})_{/ \tau_1}$ are defined using the the canonical $\Pi$-functors of $\Gamma$ isomorphisms $(G(\Gamma)_{/ \sigma})_{/ \tau_i} \cong (G(\Gamma)_{/ \operatorname{dom} \tau_i}$.
The assignment $(\Gamma, \sigma) \mapsto \Gamma_{/ \sigma}$ is functorial wrt.\@ strict functors in $\Gamma$.

Now $D : \Gamma.\sigma \rightarrow \Gamma_{/ \sigma}$ should be defined as $D = \langle \sigma^*, d \rangle$, where $\sigma^* : G(\Gamma) \rightarrow G(\Gamma_{/ \sigma})$ and $d : \sigma \rightarrow \sigma \times \sigma$ is the diagonal.
But $\sigma^*$ is not strict, and there appears to be no obvious way to modify the strict lcc structure of $\Gamma_{/ \sigma}$ to make $\sigma^*$ strict.
