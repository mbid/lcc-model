...
\begin{proof}
  ...
  We have to construct the powers such that they commute with $G$, i.e.\@ $G(X^\mathcal{I}) = G(X)^\mathcal{I}$.

  Let $j : A \rightarrow B$ be in $J$ and let $a : A \rightarrow G(X)^\mathcal{I}$.
  We have to define a canonical lift $\ell(j, a) : B \rightarrow G(X)^\mathcal{I}$.
  $a$ corresponds to a map $\bar a : \mathcal{I} \rightarrow \mathcal{M}(A, G(X))$, i.e.\@ an isomorpism of maps $A \rightarrow G(X)$.
  Its source and target are morphisms $\bar a_0, \bar a_1 : A \rightarrow G(X)$, for which we obtain canonical lifts $\ell(j, \bar a_i) : B \rightarrow G(X)$.
  ...
  It follows that $\bar a$ can be lifted uniquely to an isomorphism of $\ell(j, \bar a_0)$ and $\ell(j, \bar a_0)$, and we take $\ell(j, a) : B \rightarrow G(X)^\mathcal{I}$ to be this isomorphism's transpose.

  From the uniqueness of the lift defining $\ell(j, a)$ it follows that a map $G(Y) \rightarrow G(X)^\mathcal{I}$ preserves canonical lifts if and only if the two maps
  \begin{equation}
  ...
  For this it suffices to note that $G$ preserves (weighted) limits and that $G$ preserves and reflects fibrations and weak equivalences, so the map
  \begin{equation}
...
Dually, a morphism $f : \Delta \rightarrow \Gamma$ in a covariant should thus be thought of as a mapping of the variables in $\Delta$ to terms in context $\Gamma$, or as an interpretation of $\Delta$ in $\Gamma$.
...
To distinguish type and term formers in (covariant) cwfs from the corresponding categorical structure, the type theoretic notions are generally typeset in bold font.
...
With theorem \ref{th:algebraically-fibrant-model-category} we have found an alternative model category presenting the higher category of lcc categories, which can be equipped with meaningful contravariant cwf structure:
\begin{definition}
  The model $\mathrm{Gpd}$-category of strict lcc categories and strict lcc functors is given by $\mathrm{sLcc} = \operatorname{Alg} \mathrm{Lcc}$.
...
We thus have to related the slice category $U(G(\Gamma))_{/ \sigma}$ to the context extension $U(G(\Gamma.\sigma))$, and it appears that this not possible in general.
This issue is dealt with in section \ref{sec:algebraically-cofibrant}.
...
\begin{proposition}
  ...
  Then there is a strict lcc category $\Gamma_{/ \sigma}$ whose underlying category is $U(G(\Gamma))_{/ \sigma}$.
  ...
  \begin{equation}
    \Sigma_\sigma \dashv U(\sigma^*) \dashv \Pi_\sigma.
  \end{equation}
  These data are natural in $\Gamma$, in the sense that if $f : \Gamma \rightarrow \Delta$ is strict lcc and satisfies $f(\sigma) = \tau$, then the canonical functor $f_{/ \sigma} : \Gamma_{/ \sigma} \rightarrow \Delta_{/ \tau}$ is strict lcc, the three squares
  \begin{mathpar}
    \begin{tikzcd}
...
\begin{theorem}[\cite{coalgebraic-models} Lemmas 1.2 and 1.3, Theorems 1.4 and 2.5]
  ...
  \begin{enumerate}
    ...
    \item
      an object $X \in \mathcal{M}$ is $\lambda$-presentable if and only if the functor $\mathcal{M}(X, -) : \mathcal{M} \rightarrow \mathrm{Set}$, given by the groupoid enrichment of $\mathcal{M}$, preserves $\lambda$-filtered colimits.
      \todo{something's not quite right here}
  \end{enumerate}
...
For $\mathcal{M} = \mathrm{sLcc}$, the first infinite cardinal $\omega$ satisfies the three conditions of theorem \ref{th:coalgebraic-model-category}, and we take $C = FG$ as cofibrant replacement comonad.
