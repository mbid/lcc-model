...
\begin{abstract}
  ...
  To that end, we construct a model category of lcc sketches, from which we obtain by the formalism of algebraically (co)fibrant objects model categories of strict lcc categories and then algebraically cofibrant strict lcc categories, and the latter is our model of dependent type theory.
\end{abstract}
...
The idea different contexts should be understood as different categories is by no means novel, and indeed widespread among researchers of geometric logic; see e.g. \citet[section 4.5]{locales-and-toposes-as-spaces}.
...
\end{theorem}
Theorem \ref{th:algebraically-fibrant-model-category} appears in \citet{algebraic-models} with the additional assumption that all cofibrations in $\mathcal{M}$ are monomorphism.
This assumption is lifted in \citet{equipping-weak-equivalences}, but there $J$ is a set of generating trivial cofibrations, which is slightly stronger requirement than the one stated in the theorem.
...
\begin{proof}
  ...
  \begin{mathpar}
    ...
    \and
    \begin{tikzcd}
      f(\tau) \arrow[r, "{\langle w, \mathrm{id} \rangle}"] \arrow[d] & f(x \times y) \arrow[d, "f(\mathrm{pr}_1)"] \\
      t \arrow[r, "w"] & f(x)
    \end{tikzcd}
  \end{mathpar}
