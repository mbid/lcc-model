...
\begin{proof}
  ...
  By \cite[Theorem 4.1.1]{hirschhorn}, these are precisely the $S$-local fibrant objects of $\mathrm{Cat}^{I_\mathrm{Lcc}}$, i.e.\@ fibrant lcc marked categories $\mathcal{C}$ such that for all morphism $f : A \rightarrow B$ in $S$ the induced map
  \begin{equation}
    \mathrm{Lcc}(f, \mathcal{C}) : \mathrm{Lcc}(B, \mathcal{C}) \rightarrow \mathrm{sSet}(A, \mathcal{C})
  \end{equation}
  of groupoids is a trivial fibration.
  \todo{Rewrite the proof for groupoids.}
